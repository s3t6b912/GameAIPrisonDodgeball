    ...
    public class MinionStateMachine : MonoBehaviour
    {
        ...
        class CollectBallState : MinionState
        {
            ...
            public override DeferredStateTransitionBase<MinionFSMData> Update()
            {
                ...
                PrisonDodgeballManager.OpponentInfo[] oppInfo = new PrisonDodgeballManager.OpponentInfo[Mgr.TeamSize];
                Mgr.GetAllOpponentInfo(TeamData.Team, ref oppInfo);
                float opponentBalls = 0;
                foreach (PrisonDodgeballManager.OpponentInfo info in oppInfo)
                {
                    if (info.HasBall)
                    {
                        opponentBalls++;
                ...
                float ballRatio = opponentBalls / TeamData.NumBalls;

                // could pick up a ball accidentally before getting to desired ball
                if (Minion.HasBall && ballRatio < 0.5) {
                    return GoToThrowSpotTransition;
                }
                else if (ballRatio >= 0.5)
                {
        ...
        class DefensiveDemoState : MinionState
        {
            ...
            public override void Enter()
            {
                ...
                RandomGoTo();
                Minion.Evade(MinionScript.EvasionDirection.Left, Random.Range(1f, 5f));
            ...
            public override DeferredStateTransitionBase<MinionFSMData> Update()
            {
                ...
                if (Minion.Velocity.magnitude < 1 || Minion.ReachedTarget())
                {
                    RandomGoTo();
                    ...
                    Minion.Evade(ev, Random.Range(1f, 5f));
                }
                ...
                if (FindClosestAvailableDodgeball(out ball) && ballRatio < 0.5)
                {
                    return CollectBallTransition;
                }
